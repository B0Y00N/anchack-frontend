...
<template>
  ...
  >
    <div
      class="bg-card w-full max-w-[560px] max-h-[88vh] overflow-y-auto rounded-2xl shadow-2xl border border-border"
      @click.stop
    >
      <div
      ...
      <div v-if="submitted" class="p-12 text-center">
        ...
        <button
          @click="emit('close')"
          class="bg-primary text-primary-foreground font-semibold px-8 py-3 rounded-full hover:bg-primary/90"
        >
          닫기
        </button>
      </div>
      ...
      <div v-else class="p-6 space-y-6">
        ...
        <div>
          <label class="block text-sm font-semibold text-foreground mb-2">
            ...
            <span class="text-xs font-normal text-muted-foreground">(최소 20자)</span>
          </label>
          <textarea
            v-model="content"
          ...
          />
          <p
            :class="`text-xs mt-1 text-right ${content.length < 20 ? 'text-muted-foreground' : 'text-primary'}`"
          >
            {{ content.length }}자
          </p>
        </div>
        ...
        <div>
          <div class="flex items-center justify-between mb-3">
            ...
            >
              <span
                :class="`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${isAnonymous ? 'translate-x-5' : 'translate-x-0'}`"
              />
            </button>
          </div>
          <input
        ...
        <button
          :disabled="!canSubmit"
          @click="handleSubmit"
          class="w-full bg-primary text-primary-foreground font-bold py-3.5 rounded-xl hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed shadow-sm"
        >
          리뷰 등록하기
        </button>
