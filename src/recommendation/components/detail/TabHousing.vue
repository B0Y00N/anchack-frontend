...
<template>
  <div class="space-y-5">
    <div class="grid grid-cols-2 gap-4">
      <div class="bg-card border border-border rounded-2xl p-6"><p class="text-xs text-muted-foreground mb-2">보증금 중위값 (전체 유형)</p><p class="text-3xl font-bold text-foreground">{{ n.deposit.toLocaleString() }}만원</p></div>
      <div class="bg-card border border-border rounded-2xl p-6"><p class="text-xs text-muted-foreground mb-2">월세 중위값 (전체 유형)</p><p class="text-3xl font-bold text-foreground">{{ n.monthly }}만원</p></div>
    </div>
