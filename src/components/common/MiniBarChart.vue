...
<template>
  <div class="flex items-end gap-3" :style="{ height: height + 'px' }">
    <div v-for="d in data" :key="d.label" class="flex-1 flex flex-col items-center justify-end h-full">
      <span class="text-xs font-semibold text-foreground mb-1">{{ d.value }}{{ d.unit || "" }}</span>
      <div class="w-full rounded-t-md transition-all" :style="{ height: (d.value / max) * (height - 40) + 'px', background: barColor(d.value) }" />
