...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" @click="emit('close')">
    <div class="bg-card w-full max-w-[520px] rounded-2xl shadow-2xl border border-border overflow-hidden" @click.stop>
      <div class="px-6 pt-6 pb-4 border-b border-border">
        ...
        <div class="flex gap-2">
          <div class="flex-1 relative">
            ...
            <input autofocus type="text" v-model="query" placeholder="회사명, 학교명, 도로명 주소 검색" class="w-full bg-muted rounded-xl pl-9 pr-4 py-3 text-sm border-0 outline-none focus:ring-2 focus:ring-primary/30" />
          </div>
        </div>
      </div>
      <div class="max-h-[360px] overflow-y-auto [&::-webkit-scrollbar]:hidden">
        <div v-if="filtered.length === 0" class="py-12 text-center text-sm text-muted-foreground">검색 결과가 없어요</div>
        <ul v-else class="divide-y divide-border/60">
          <li v-for="(item, i) in filtered" :key="i">
            <button @click="emit('select', item.name, item.address)" class="w-full text-left px-6 py-4 hover:bg-secondary transition-colors flex items-start gap-3">
      ...
      <div class="px-6 py-3 bg-muted/40 border-t border-border">
        <p class="text-xs text-muted-foreground">주소가 없다면 직접 입력 후 다음 단계로 넘어가주세요.</p>
      </div>
    </div>
  </div>
</template>
