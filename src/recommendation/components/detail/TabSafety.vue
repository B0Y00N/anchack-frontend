...
<template>
  <div class="space-y-5">
    ...
    <div class="bg-card border border-border rounded-2xl p-6">
      ...
      <div class="space-y-3">
        <ScoreBar v-for="item in bars" :key="item.label" :label="item.label" :value="item.value" unit="점" :color="scoreColor(item.value)" />
      </div>
    </div>
    <div class="bg-card border border-border rounded-2xl p-5 flex items-start gap-3">
      ...
      <div><p class="font-semibold text-foreground text-sm mb-1">가까운 경찰관서</p><p class="text-sm text-foreground/80">{{ n.police }}</p></div>
    </div>
