    ...
    <div class="bg-card w-full max-w-[520px] max-h-[85vh] overflow-y-auto rounded-2xl shadow-2xl border border-border" @click.stop>
      ...
      <div class="p-6 space-y-6">
        ...
        <div>
          ...
          <textarea v-model="content" rows="4" maxlength="500" class="w-full bg-muted rounded-xl px-4 py-3 text-sm border-0 outline-none focus:ring-2 focus:ring-primary/30 resize-none" />
          <p :class="`text-xs mt-1 text-right ${content.trim().length < 20 ? 'text-muted-foreground' : 'text-primary'}`">{{ content.length }}/500자</p>
        </div>

        <div class="flex items-center justify-between">
          <div>
            <label class="text-sm font-semibold text-foreground">익명으로 표시</label>
            <p class="mt-0.5 text-xs text-muted-foreground">{{ isAnonymous ? "작성자 정보가 익명으로 표시돼요" : "닉네임으로 게시돼요" }}</p>
          </div>
          <button
            type="button"
            role="switch"
            :aria-checked="isAnonymous"
            :class="['relative h-6 w-11 flex-shrink-0 rounded-full transition-colors', isAnonymous ? 'bg-primary' : 'bg-muted-foreground/30']"
            @click="isAnonymous = !isAnonymous"
          >
            <span :class="['absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform', isAnonymous ? 'translate-x-5' : 'translate-x-0']" />
          </button>
        </div>
