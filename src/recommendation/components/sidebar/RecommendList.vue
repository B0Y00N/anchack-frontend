...
<template>
  <div class="w-[430px] flex-shrink-0 flex flex-col border-r border-border bg-background">
    <div class="px-6 pt-6 pb-4 border-b border-border">
      <div v-if="zeroResults" class="flex items-center gap-2 text-amber-600"><AlertCircle :size="18" /><h2 class="text-lg font-bold">검색 결과 0건</h2></div>
      <template v-else>
        <h2 class="text-xl font-bold text-foreground mb-1.5">석원님에게 잘 맞는 동네 5곳을 찾았어요</h2>
        <p class="text-sm text-muted-foreground">추천 이유와 아쉬운 점을 함께 비교해보세요.</p>
      </template>

      <div v-if="!zeroResults" class="mt-3 flex items-center gap-2">
    ...
    <div v-if="!zeroResults" class="px-5 py-3 flex gap-2 overflow-x-auto border-b border-border [&::-webkit-scrollbar]:hidden">
      <button
        v-for="f in FILTERS"
        :key="f"
    ...
    <div v-if="compareList.length >= 2 && !zeroResults" class="px-5 py-3 bg-secondary flex items-center justify-between border-b border-primary/15">
    ...
    <div class="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden">
      <div v-if="zeroResults" class="p-4">
