...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" @click="emit('close')">
    <div class="bg-card w-full max-w-[460px] rounded-2xl shadow-2xl border border-border overflow-hidden" @click.stop>
      ...
      <div class="max-h-[420px] overflow-y-auto [&::-webkit-scrollbar]:hidden">
        <div v-if="savedConditions.length === 0" class="py-14 text-center text-sm text-muted-foreground">저장된 조건이 없어요.</div>
            ...
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                ...
                <div class="flex flex-wrap gap-1.5 mt-2">
                  <span v-if="c.state.addressTab === 'known' && c.state.detailAddress" class="inline-flex items-center gap-1 text-[11px] bg-muted px-2 py-0.5 rounded-full text-foreground/70">
                    <MapPin :size="9" /> {{ c.state.detailAddress }}
                  </span>
                  <span v-if="c.state.monthly > 0" class="inline-flex items-center gap-1 text-[11px] bg-muted px-2 py-0.5 rounded-full text-foreground/70">월세 {{ c.state.monthly }}만원 이하</span>
                  <span v-for="p in c.state.priorities.slice(0, 2)" :key="p" class="inline-flex items-center gap-1 text-[11px] bg-primary/10 text-primary px-2 py-0.5 rounded-full">{{ p }}</span>
                </div>
              </div>
