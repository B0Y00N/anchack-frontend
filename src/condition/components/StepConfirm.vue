...
const depositChips = computed(() =>
  props.state.rentType === '월세'
...
function toggleMustHave(m) {
  const s = props.state
  update({
    mustHave: s.mustHave.includes(m) ? s.mustHave.filter((x) => x !== m) : [...s.mustHave, m],
  })
}
function toggleHousing(h) {
  const s = props.state
  update({
    housingTypes: s.housingTypes.includes(h)
      ? s.housingTypes.filter((x) => x !== h)
      : [...s.housingTypes, h],
  })
}
...
<template>
        ...
        <div v-if="state.addressTab === 'known'" class="space-y-2">
          <div class="flex gap-2">
            <input
              ...
              placeholder="우편번호"
              @click="modal = true"
              class="w-28 bg-white border border-border rounded-xl px-4 py-3 text-sm cursor-pointer"
            />
            <button
              @click="modal = true"
              class="bg-primary text-primary-foreground px-4 py-3 rounded-xl text-sm font-semibold flex items-center gap-1.5 hover:bg-primary/90"
            >
              <Search :size="15" /> 우편번호 찾기
            </button>
          </div>
          <input
            type="text"
            readonly
            :value="state.commuteArea"
            placeholder="우편번호 찾기로 기본 주소를 선택해주세요"
            @click="modal = true"
            class="w-full bg-white border border-border rounded-xl px-4 py-3 text-sm cursor-pointer"
          />
          <input
            type="text"
            :value="state.detailAddress"
            placeholder="상세 주소 (선택)"
            @input="update({ detailAddress: $event.target.value })"
            class="w-full bg-white border border-border rounded-xl px-4 py-3 text-sm"
          />
        </div>
        <div
          v-if="state.addressTab === 'unknown'"
          class="bg-white border border-border rounded-xl p-4"
        >
          ...
          <DistrictMap
            :model-value="state.commuteAreas"
            @update:model-value="(areas) => update({ commuteAreas: areas })"
            :max="2"
          />
        </div>
        <div v-if="state.addressTab === 'known'" class="space-y-2">
          ...
          <div class="flex gap-2 flex-wrap">
            <BoxChip
              v-for="t in [30, 45, 60, 75, 90]"
              :key="t"
              :label="`${t}분`"
              :selected="state.maxCommuteTime === t"
              @click="update({ maxCommuteTime: t })"
            />
          </div>
        </div>
      </div>
      ...
      <div class="space-y-4">
        <div>
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-secondary text-primary"
              >{{ state.priorities.length }}/3</span
            >
          </div>
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            <MustHaveChip
              v-for="m in MUST_HAVE_OPTIONS"
              :key="m"
              :label="m"
              :selected="state.mustHave.includes(m)"
              @click="toggleMustHave(m)"
            />
          </div>
        </div>
      </div>
    ...
    >
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs text-muted-foreground mb-1"
              >{{ state.rentType === '월세' ? '최대 보증금' : '최대 전세금' }} (만원)</label
            >
            <input
              type="number"
              step="10"
              :value="state.deposit"
              @input="update({ deposit: +$event.target.value })"
              class="w-full bg-white border border-border rounded-xl px-4 py-3 text-sm"
            />
          </div>
          <div v-if="state.rentType === '월세'">
            ...
            <input
              type="number"
              :value="state.monthly"
              @input="update({ monthly: +$event.target.value })"
              class="w-full bg-white border border-border rounded-xl px-4 py-3 text-sm"
            />
          </div>
        </div>
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            <Chip
              v-for="v in depositChips"
              :key="v"
              :label="`${v.toLocaleString()}만원`"
              :selected="state.deposit === v"
              @click="update({ deposit: v })"
            />
          </div>
        </div>
      </div>
    ...
    >
      <div class="space-y-4">
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            <Chip
              v-for="h in HOUSING_OPTIONS"
              :key="h"
              :label="h"
              :selected="state.housingTypes.includes(h)"
              @click="toggleHousing(h)"
            />
          </div>
        </div>
        <div>
  ...
  <div class="flex justify-between">
    ...
    <button
      @click="emit('submit')"
      class="bg-primary text-primary-foreground px-10 py-3.5 rounded-full font-semibold"
    >
      동네 찾기 시작
    </button>
  </div>
